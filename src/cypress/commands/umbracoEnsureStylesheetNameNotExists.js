...
export default class UmbracoEnsureStylesheetNameNotExists extends CommandBase {
  ...
  method(name) {
    ...
    cy.getCookie("UMB-XSRF-TOKEN", {log:false}).then(token => {
      cy.request({
        method: 'GET',
        url: this._relativeBackOfficePath + '/BackOffice/Api/StylesheetsTree/GetNodes?id=-1',
        followRedirect: true,
        headers: {
        ...
        if(searchBody.length > 0){
          ...
          if(partialViewId !== null){
            cy.request({
              method: 'POST',
              url: this._relativeBackOfficePath + '/backoffice/UmbracoApi/CodeFile/Delete?type=stylesheets&virtualPath=' + partialViewId,
